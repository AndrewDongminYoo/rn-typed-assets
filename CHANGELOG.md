...
- **Collision policy (`onCollision`)** — when two files normalize to the same key, the new `onCollision` config field / `--on-collision <error|first>` flag selects the behavior: `error` (default) preserves the existing fail-fast behavior, while `first` keeps the first sorted path, drops the rest, and reports each dropped file in `data.collisions` (JSON) or as `Collision:` lines (text). The policy is non-interactive by design so it stays compatible with `--output json`.
